...
| OpenClaw/Hermes | Native adapters and a standalone live soak evaluator | OpenClaw native hook was revalidated on `.14`; Hermes provider can propose and retrieve accepted memory, but Hermes/Qwen one-shot did not reliably use its recalled fact in a final answer on 2026-07-12. Deterministic native-runtime consumption evidence and multi-hour soak remain required. |
