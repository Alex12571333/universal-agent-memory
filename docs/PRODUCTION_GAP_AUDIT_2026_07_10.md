...
| Audit trail | `audit_events`, export/signing and retention tools exist; retain, CAS supersede, proposal submission/reviews, curation proposals and conflict decisions share their canonical write transaction | Coverage is incomplete for raw conversation append, graph, reflection, reindex and checkpoints |
...
7. `memory.retain` and `memory.supersede` audit records are now committed in
   ...
   Conversation curation uses the same transaction for
   `conversation.curate.propose`, so an audit failure leaves no LLM-derived
   proposal. Denied requests, raw conversation append, graph writes, reflect,
