...
Локальная operator UI доступна на `http://localhost:8080/ui`. Это русский
glass-style dashboard с KPI, recall, conflict inbox, редактируемым
Obsidian-style vault, интерактивным графом памяти с drag/zoom и web-настройками
embedding моделей. Если задан `UAM_API_KEY`, UI защищена тем же bearer-key
middleware, что и API. Подробности: [docs/WEB_DASHBOARD.md](docs/WEB_DASHBOARD.md).
