...
Локальная operator UI доступна на `http://localhost:8080/ui`. Это React/Vite
glass-style dashboard с KPI, recall, conflict inbox, редактируемым
Obsidian-style vault, интерактивным drag-графом памяти и web-настройками
embedding моделей. Docker собирает фронтенд автоматически. Если задан
`UAM_API_KEY`, UI защищена тем же bearer-key middleware, что и API. Подробности:
[docs/WEB_DASHBOARD.md](docs/WEB_DASHBOARD.md).

Глубокая интеграция в OpenClaw/Hermes описана в
[docs/AGENT_INTEGRATION.md](docs/AGENT_INTEGRATION.md): какие lifecycle hooks
вызывают recall/retain/checkpoint и как memory context попадает в модель.
